...
delivery. A finished ordinary child closes its dedicated Herdr tab, and an already missing pane or tab is an idempotent cleanup success. An autonomous owner cannot publish an intermediate settled response while it still owes descendant processing: the descendant result wakes it, and a new eligible `agent_settled` boundary publishes the owner's answer. Confirmed manual pane closure is destructive ownership evidence: a surviving ancestor adopts every open descendant, terminates or terminalizes the branch from the leaves upward, and delivers one failure for the destroyed owner. Unknown Herdr inspection remains pending.
