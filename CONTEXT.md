...
A **descendant** is a child launched by a subagent. The **logical immediate parent** is the exact session or workflow run that owns its result. A node is **drained** only when its terminal outcome is delivered to that sink and all descendants are drained recursively. Settled turns stay local to the child. Only its latest terminal result can cross into the parent, once. A closed parent session keeps an offline inbox; the result materializes only when that exact session is restored. Workflow node results stay in a run inbox; after a coordinator restart, nodes drain first and exactly one interrupted envelope is published without replaying the script. **Cleanup** is separate from delivery. A finished ordinary child closes its dedicated Herdr tab, and an already missing pane or tab is an idempotent cleanup success. Confirmed manual pane closure is destructive ownership evidence: a surviving ancestor adopts every open descendant, terminates or terminalizes the branch from the leaves upward, and delivers one failure for the destroyed owner. Unknown Herdr inspection remains pending.
