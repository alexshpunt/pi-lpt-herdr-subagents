...
When an autonomous child process crashes inside its still-owned tab or a leaf becomes stale, the extension resumes the exact session automatically. It reuses the pane when safe and otherwise creates a replacement in the same Herdr workspace. Every ordinary replacement tab is owned from creation and cleaned if setup fails. Crash and stale recovery share one durable three-attempt budget for each child. Successful recovery stays silent in the parent and appears in the widget. If the budget runs out, the exact parent receives one `recovery-failed` result with the child session and result-file paths. Recovery state survives extension reloads and parent restarts.

Closing a subagent tab is different from a process crash. After a short grace period for racing completion evidence, the extension treats a missing owned tab as explicit user cancellation. It never resumes that subagent or spends its recovery budget, stops owned descendants from the leaves upward, retains worktree files, and sends the exact creator one durable `cancelled` result. Ambiguous Herdr inspection never causes a guessed cancellation or watchdog kill.
