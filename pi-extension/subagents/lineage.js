...
export function latestDescendantDeliveryAt(state, nodeId) {
    const integrity = lineageIntegrityError(state);
    if (integrity)
        throw new Error(integrity);
    let latest;
 ...
 */
export function lineageFromEnvironment(env = process.env, parentPid = process.ppid) {
    if (!env.PI_SUBAGENT_LINEAGE_DIR ||
