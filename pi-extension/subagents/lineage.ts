...
/** Durable launch registration and recursive ownership projection. */
export interface LineageRegistration {
  rootDir: string;
  rootId: string;
  nodeId: string;
  parentNodeId?: string;
  ...
  launchKind: "fresh" | "resume" | "worktree" | "fork" | "workflow";
}
...
export interface LineageState {
  rootId: string;
  nodes: Map<string, LineageNodeState>;
  events: number;
}
...
interface LineageEvent {
  ...
  type: string;
  nodeId: string;
  /** Ordering is informational only. Reduction never depends on this clock. */
  at: number;
  rootId?: string;
  [key: string]: unknown;
}
...
function eventDir(rootDir: string): string {
...
export function registerLineage(options: {
  ...
  parentSessionFile?: string;
  parentWorkflowRunId?: string;
  launchKind?: LineageRegistration["launchKind"];
  inheritedRootDir?: string;
...
export function appendLineageEvent(
  ...
  if (!eventId || !type || !nodeId) return false;
...
function validEvent(value: unknown, fileName?: string, rootId?: string): value is LineageEvent {
  const e = value as Partial<LineageEvent> | null;
  if (!e || e.version !== 1 || typeof e.eventId !== "string" || !e.eventId ||
      typeof e.type !== "string" || !e.type || typeof e.nodeId !== "string" ||
      !e.nodeId || !Number.isFinite(e.at)) return false;
  if (rootId && e.rootId !== undefined && e.rootId !== rootId) return false;
  if (fileName && fileName !== `${safeId(e.eventId)}.json`) return false;
  return true;
}
...
export function reduceLineage(rootDir: string): LineageState {
  const rootId = rootDir.split(/[\\/]/).pop() ?? "unknown";
  const nodes = new Map<string, LineageNodeState>();
  let events = 0;
  let files: string[] = [];
  try { files = readdirSync(join(rootDir, "events")).filter((f) => f.endsWith(".json")); } catch { return { rootId, nodes, events }; }
  const parsed: LineageEvent[] = [];
  for (const file of files) {
    try {
      const value = JSON.parse(readFileSync(join(rootDir, "events", file), "utf8"));
      if (validEvent(value, file, rootId)) parsed.push(value);
    } catch {}
  }
  parsed.sort((a, b) => (eventRank[a.type] ?? 50) - (eventRank[b.type] ?? 50) || a.eventId.localeCompare(b.eventId));
  const launched = new Set<string>();
  for (const event of parsed) {
    if (event.type !== "launch" && !launched.has(event.nodeId)) continue;
    ...
    if (event.type === "launch") {
      ...
      if (!node.parentNodeId && typeof event.parentNodeId === "string") node.parentNodeId = event.parentNodeId;
      if (!node.parentSessionId && typeof event.parentSessionId === "string") node.parentSessionId = event.parentSessionId;
      if (!node.parentSessionFile && typeof event.parentSessionFile === "string") node.parentSessionFile = event.parentSessionFile;
      if (!node.parentWorkflowRunId && typeof event.parentWorkflowRunId === "string") node.parentWorkflowRunId = event.parentWorkflowRunId;
      if (!node.launchKind && typeof event.launchKind === "string") node.launchKind = event.launchKind;
      ...
      if (node.startTime !== undefined && runStart < node.startTime) continue;
      if (typeof event.name === "string") node.name = event.name;
      ...
      prior.intent = true;
      if (typeof event.surface === "string") prior.surface = event.surface;
      if (Array.isArray(event.pids)) {
        const pids = event.pids.filter((pid): pid is number => Number.isInteger(pid) && pid > 0);
        if (pids.length > 0) prior.pids = [...new Set(pids)];
      }
      if (event.type === "cancel_proven") prior.proven = true;
      ...
      if (node.terminalDelivered) node.cleanupPending = false;
    }
    events++;
  }
...
export function appendLineageInbox(
  rootDir: string,
  nodeId: string,
  deliveryId: string,
  sink: { sessionId?: string; sessionFile?: string; workflowRunId?: string },
  payload: Record<string, unknown> = {},
): boolean {
  if (!sink.workflowRunId && (!sink.sessionId || !sink.sessionFile)) return false;
  const eventId = `inbox:${deliveryId}`;
  const file = join(eventDir(rootDir), `${safeId(eventId)}.json`);
  try {
    const existing = JSON.parse(readFileSync(file, "utf8")) as Partial<LineageEvent>;
    if (validEvent(existing, `${safeId(eventId)}.json`, rootDir.split(/[\\/]/).pop()) && existing.type === "inbox" && existing.nodeId === nodeId) return true;
  } catch {}
...
export interface LineageInboxRecord {
  rootDir: string;
  nodeId: string;
  deliveryId: string;
  sessionId?: string;
...
/** Stable identity for one process incarnation. A missing start time is conservative. */
export interface ProcessIncarnation {
  pid: number;
  startTime?: string;
}

interface MaterializationClaimRecord {
  version: 1;
  deliveryId: string;
  nodeId: string;
  token: string;
...
interface MaterializationGateRecord {
  version: 1;
  token: string;
  process: ProcessIncarnation;
  claimedAt: number;
...
function materializationClaimDir(rootDir: string): string {
...
function claimFile(rootDir: string, deliveryId: string): string {
...
function currentProcessIncarnation(): ProcessIncarnation {
  const startTime = processStartTime(process.pid);
  return { pid: process.pid, ...(startTime ? { startTime } : {}) };
}

function exactSessionEvidence(check: () => boolean): boolean {
  try { return check() === true; } catch { return false; }
}

function readMaterializationClaim(path: string, deliveryId: string, nodeId: string): MaterializationClaimRecord | undefined {
  try {
...
function readMaterializationGate(path: string): MaterializationGateRecord | undefined {
  try {
    const value = JSON.parse(readFileSync(path, "utf8")) as Partial<MaterializationGateRecord>;
    if (value.version !== 1 || typeof value.token !== "string" || !value.process ||
        ...
        !Number.isFinite(value.claimedAt)) return undefined;
    return value as MaterializationGateRecord;
  } catch {
    return undefined;
  }
}

function processIncarnationAlive(identity: ProcessIncarnation, observer: ProcessIncarnation): boolean {
  ...
  if (!identity.startTime || !observed.startTime) return true;
  return identity.startTime === observed.startTime;
}

function materializationGateFile(rootDir: string): string {
  return join(materializationClaimDir(rootDir), "lineage.gate");
}

function acquireMaterializationGate(rootDir: string, observer: ProcessIncarnation): { token: string } | null {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (current && processIncarnationAlive(current.process, observer)) return null;
    try {
...
function releaseMaterializationGate(rootDir: string, token: string): void {
  const path = materializationGateFile(rootDir);
  const current = readMaterializationGate(path);
  if (current?.token === token) try { unlinkSync(path); } catch {}
}

function readLineageInboxes(rootDir: string): { record: LineageInboxRecord; materialized: boolean }[] {
  const result: { record: LineageInboxRecord; materialized: boolean }[] = [];
  const materialized = new Set<string>();
  let files: string[] = [];
  try { files = readdirSync(join(rootDir, "events")).filter((f) => f.endsWith(".json")); } catch { return result; }
  const records: LineageInboxRecord[] = [];
  for (const file of files) {
    try {
      const value = JSON.parse(readFileSync(join(rootDir, "events", file), "utf8"));
      if (!validEvent(value, file, rootDir.split(/[\\/]/).pop())) continue;
      if (value.type === "inbox_materialized" && typeof value.deliveryId === "string") materialized.add(value.deliveryId);
      if (value.type !== "inbox" || typeof value.deliveryId !== "string") continue;
...
function sameMaterializationSink(left: LineageInboxRecord, right: LineageInboxRecord): boolean {
...
function pendingBefore(rootDir: string, deliveryId: string): boolean {
...
export function claimLineageInboxMaterialization(options: {
  ...
  try {
    ...
    if (existing) {
      ...
      if (processIncarnationAlive(existing.process, observer)) return { status: "busy" };
      try {
        const tombstone = `${path}.${process.pid}.${randomUUID()}.stale`;
        renameSync(path, tombstone);
        try { unlinkSync(tombstone); } catch {}
    ...
    if (pendingBefore(rootDir, deliveryId)) return { status: "busy" };
    const token = randomUUID();
...
export function completeLineageInboxMaterialization(
  rootDir: string,
  deliveryId: string,
  nodeId: string,
  token: string,
...
export function releaseLineageInboxMaterialization(options: {
  ...
  if (isLineageInboxMaterialized(options.rootDir, options.deliveryId)) return true;
  if (exactSessionEvidence(options.hasExactSessionEvidence)) {
  ...
  if (claim?.token !== options.token) return false;
  try { unlinkSync(path); } catch {}
  return true;
}

/** Return whether an inbox has a durable materialization acknowledgement. */
export function isLineageInboxMaterialized(rootDir: string, deliveryId: string): boolean {
  let files: string[] = [];
  try { files = readdirSync(join(rootDir, "events")).filter((f) => f.endsWith(".json")); } catch { return false; }
  for (const file of files) {
    try {
      const value = JSON.parse(readFileSync(join(rootDir, "events", file), "utf8"));
      if (validEvent(value, file, rootDir.split(/[\\/]/).pop()) && value.type === "inbox_materialized" && value.deliveryId === deliveryId) return true;
    } catch {}
  }
  return false;
}
...
export function hasLineageEvent(rootDir: string, eventId: string, type?: string): boolean {
  const file = join(eventDir(rootDir), `${safeId(eventId)}.json`);
  try {
...
export function pendingLineageInboxes(rootDir: string, sessionId: string, sessionFile: string): LineageInboxRecord[] {
  const pending: LineageInboxRecord[] = [];
  let files: string[] = [];
  try { files = readdirSync(join(rootDir, "events")).filter((f) => f.endsWith(".json")); } catch { return pending; }
  const materialized = new Set<string>();
  const inboxes: LineageInboxRecord[] = [];
  for (const file of files) {
    try {
      const value = JSON.parse(readFileSync(join(rootDir, "events", file), "utf8"));
      if (!validEvent(value, file, rootDir.split(/[\\/]/).pop())) continue;
  ...
  for (const inbox of inboxes) if (!materialized.has(inbox.deliveryId)) pending.push(inbox);
...
export function readLineageAttachment(sessionFile: string): LineageRegistration | undefined {
  try {
    const value = JSON.parse(readFileSync(`${sessionFile}.lineage.json`, "utf8")) as Partial<LineageRegistration>;
    if (typeof value.rootDir !== "string" || typeof value.rootId !== "string" || typeof value.nodeId !== "string") return undefined;
    if (!existsSync(join(value.rootDir, "events"))) return undefined;
    return value as LineageRegistration;
  } catch {
    return undefined;
  }
}
...
export function discoverLineageRoots(sessionDir: string, sessionFile?: string): string[] {
  const result = new Set<string>();
  if (sessionFile) {
    const attachment = readLineageAttachment(sessionFile);
    if (attachment) result.add(attachment.rootDir);
  }
  try {
  ...
  try {
    for (const artifact of readdirSync(join(sessionDir, "artifacts"), { withFileTypes: true })) {
      ...
      for (const root of readdirSync(lineageDir, { withFileTypes: true })) {
        if (root.isDirectory()) result.add(join(lineageDir, root.name));
      }
    }
  } catch {}
  return [...result].sort();
