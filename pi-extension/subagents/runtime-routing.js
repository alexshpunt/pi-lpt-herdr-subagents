...
export function parseExactModelRef(reference) {
    ...
    return {
        ...
        hasConfiguredAuth (model) {
            if (!registry.hasConfiguredAuth) {
                return (registry.getAvailable?.() ?? []).some((candidate)=>candidate.provider === model.provider && candidate.id === model.id);
            }
...
export function parseModelFallbacks(reference) {
    const candidates = reference.split(",").map((candidate)=>candidate.trim());
    if (candidates.some((candidate)=>candidate === "")) {
        throw new RuntimeResolutionError(`model fallback list ${JSON.stringify(reference)} cannot contain an empty candidate`);
    }
    return candidates;
}
export function resolveRuntimePlan(request, agentDefaults, parent, registry) {
    ...
    if (modelSelection.value) {
        ...
        if (!parsed) {
            throw new RuntimeResolutionError(`model ${JSON.stringify(modelSelection.value)} must be an exact authenticated provider/model-id`);
        }
        ...
        if (!found) {
            ...
            throw new RuntimeResolutionError(`unknown model ${JSON.stringify(modelSelection.value)}; exact registry match required. Available: ${alternatives.join(", ") || "(none)"}`);
        }
        if (!registry.hasConfiguredAuth(found)) {
            throw new RuntimeResolutionError(`model ${JSON.stringify(modelSelection.value)} has no configured authentication`);
        }
    ...
    if (!isThinkingLevel(preferredThinking)) {
        throw new RuntimeResolutionError(`thinking ${JSON.stringify(preferredThinking)} must be one of: ${THINKING_LEVELS.join(", ")}`);
    }
    let thinking = preferredThinking;
    let thinkingAdjustment;
    if (thinkingSelection.source !== "parent") {
        if (!selectedModel) {
            throw new RuntimeResolutionError(`model capability information is unavailable; cannot validate explicit thinking ${JSON.stringify(preferredThinking)}`);
        }
        const supported = getSupportedThinkingLevels(asPiModel(selectedModel));
        if (!supported.includes(preferredThinking)) {
            throw new RuntimeResolutionError(`thinking ${JSON.stringify(preferredThinking)} is not supported by ${JSON.stringify(`${provider}/${modelId}`)}; supported: ${formatSupported(selectedModel)}`);
        }
    ...
    return {
        ...
        ...modelSelection.value ? {
            requestedModel: modelSelection.value
        } : {},
        ...thinkingSelection.value ? {
            requestedThinking: preferredThinking
        } : {},
        ...thinkingAdjustment ? {
            thinkingAdjustment
        } : {}
...
export function resolveRuntimePlans(request, agentDefaults, parent, registry) {
    const selection = selectField(request.model, agentDefaults.model);
    if (!selection.value) {
        return [
            resolveRuntimePlan(request, agentDefaults, parent, registry)
        ];
    }
    return parseModelFallbacks(selection.value).map((model)=>resolveRuntimePlan(selection.source === "request" ? {
            ...request,
            model
        } : {
            ...request,
...
export function buildAuthenticatedModelCatalog(registry, limit = 24) {
    ...
    lines.push("Default: inherit the parent model and thinking. Override thinking first; override model only when task capability, speed, cost, modality, or context warrants it.");
    return lines.join("\n");
}
