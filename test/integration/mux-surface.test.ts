...
for (const backend of backends) {
  describe(`herdr terminal [${backend}]`, { timeout: 60_000 }, () => {
    ...
    it("creates an isolated worktree workspace without stealing focus", async () => {
      ...
      const worktree = createSubagentWorktree(`worktree-${id}`, env.dir, branch, baseSha);
      try {
        ...
        assert.ok(listedWorktree, `Expected worktree list to include ${worktree.path}`);
    ...
    it("sends a long command via script file without truncation", async () => {
      ...
      const screen = await waitForScreen(surface, new RegExp(`LONG_${marker}`), 15_000, 50);
      assert.ok(
